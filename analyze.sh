...
trivy image --security-checks vuln -f sarif     -o "$path/trivy.sarif.json"     "$1"
trivy image --security-checks vuln -f cyclonedx -o "$path/trivy.cyclonedx.json" "$1"
trivy image --security-checks vuln -f spdx      -o "$path/trivy.spdx.json"      "$1"
trivy image --security-checks vuln -f json      -o "$path/trivy.json"           "$1"
